...
import gov.uk.justice.digital.icantbelieveitsnotdelius.actor.{DeliusPusher, JobSchedulerPusher, Poller}
...
object Server extends App with Logging {
  ...
  def run(config: Module = new Configuration) = {
    ...
    startActor[DeliusPusher]
    startActor[JobSchedulerPusher]
    startActor[Poller]
